...
# D = torch.jit.script(_EncoderEmbedding(encoder, encoded_dim, chnl_dim))
...
# non-linear merge is just a bunch of dense ResNets 
M1 = _NonLinearMergeConv(chnl_dim, 3, 6, dropout_bool=True).to(device)
M2 = _NonLinearScoreConv(chnl_dim, 3, 6, dropout_bool=True).to(device)
