...
# we create the permutation index
perm_idx = np.random.permutation(np.linspace(0, n_samples-1, n_samples, dtype = np.int64)) 
...
class _Model(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        ----------
        x : torch.Tensor
            tensor with the four sequences appended 
        Returns
        -------
        torch.Tensor
...
for epoch in range(1, nEpochs + 1):
    ...
    train_accuracy = Tcorrect / Ttotal
    # print('number of correct testing inferences %d'%Tcorrect)
    # print('total number of testing inferences %d'%Ttotal)
    # print(train_accuracy)
    train_loss = train_loss/len(dataloaderTrain)
    print('Epoch: {} \tLearning rate: {:.6f} \tTraining Loss: {:.6f} \t Train Accuracy {:.6f} \tTime Elapsed: {:.6f}[s]'.format(
        epoch, 
    ...
    if train_accuracy >maxTAccuracy:
      maxTAccuracy = train_accuracy
