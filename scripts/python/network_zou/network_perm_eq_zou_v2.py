...
class _Model(torch.nn.Module):
    ...
    def __init__(self, embeding_dim = 80):
        ...
        self.conv = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Conv1d(embeding_dim, 32, 1),
    ...
    def forward(self, x):
        ...
        x1 = self.conv(g[:,0,:,:]).squeeze(dim=2)
        x2 = self.conv(g[:,1,:,:]).squeeze(dim=2)
        x3 = self.conv(g[:,2,:,:]).squeeze(dim=2)
