...
# drop out layers for the classification
if "dropout class" in dataJson:
    dropout_bool_class = dataJson["dropout class"]
else :
    dropout_bool_class = True
...
embeding_dim = 10 # this one is for each aminoacid

class _DoubleEmbedding(torch.nn.Module):
    # we use first an embedding for the 

    def __init__(self, length_dict, embeding_dim, chnl_dim, embd_dim, trunc_length = 1550):
        super().__init__()
        
        self.embedding_layer = nn.Embedding(length_dict, embeding_dim)
        self.non_linear_embedding = _NonLinearEmbeddingConv(trunc_length, embeding_dim, chnl_dim, embd_dim)

    def forward(self, x):

        x = self.embedding_layer(x).permute([0, 2, 1])
        x = self.non_linear_embedding(x)

        # return as a 1D vector 
        return x


D = _DoubleEmbedding(len(dict_amino), embeding_dim, chnl_dim, embd_dim, trunc_length)
    
# non-linear merge is just a bunch of dense ResNets 
M1 = _NonLinearMergeConv(chnl_dim, 3, 6, dropout_bool=dropout_bool_class)

M2 = _NonLinearScoreConv(chnl_dim, 3, 6, dropout_bool=dropout_bool_class)
