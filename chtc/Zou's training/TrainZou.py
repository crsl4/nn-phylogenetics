...
for epoch in range(1, nEpochs + 1):
    ...
    if(sample_slice < 26):
      inputTrain  = torch.Tensor(sequences[sample_slice*1000:(sample_slice+2)*1000, :, :])
      outputTrain = torch.tensor(labels[sample_slice*1000:(sample_slice+2)*1000])
    elif(sample_slice == 26):
      firstseq_tensor = torch.Tensor(sequences[26000:27000, :, :])
      secondseq_tensor = torch.Tensor(sequences[0:1000, :, :])
      inputTrain = torch.cat((firstseq_tensor,secondseq_tensor),0)
      firstlab_tensor = torch.Tensor(labels[26000:27000])
      secondlab_tensor = torch.Tensor(labels[0:1000])
      outputTrain = torch.cat((firstlab_tensor,secondlab_tensor),0)
    else:
      inputTrain  = torch.Tensor(sequences[(sample_slice-27)*1000:(sample_slice-25)*1000, :, :])
      outputTrain = torch.tensor(labels[(sample_slice-27)*1000:(sample_slice-25)*1000])
    ...
    if epoch % 10 == 0:
        ...
        for index in range(0,27):
          ...
          eachseq_lba = sequences[index*1000:(index+1)*1000, :, :]
          random_index = np.random.uniform(0,1000,50)
          random_int_index = random_index.astype(int)
          testlabel = eachlab_lba[random_int_index]
          testsequence = eachseq_lba[random_int_index,:,:]
        outputTest = torch.tensor(testlabel)
        inputTest  = torch.Tensor(testsequence)
        datasetTest = data.TensorDataset(inputTest, outputTest)
        dataloaderTest = torch.utils.data.DataLoader(datasetTest,
                                                     batch_size=batch_size,
                                                     shuffle=True,
