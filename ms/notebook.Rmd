...
# Initial idea: use neural network to approximate posterior distribution of phylogenetic tree
...
# Second idea: based on Zou2019, use NN to classify sequences based on quartet species tree they come from

Parameters:
- $L$: sequence length
- $S_t$: quartet species tree with branch length $t$
- $m$: number of quartet gene trees to simulate (this translates into sample size in training data)

## Procedure

1. Under $S_t$, simulate $m$ quartet gene trees with the coalescent model
2. For each quartet gene tree, simulate sequences of length $L$ under the continuous-time markov chain model
3. Use the $m$ sets of sequences with corresponding "label" (quartet species tree where they came from) to train a neural network model
